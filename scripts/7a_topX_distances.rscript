...
option_list <- list(
  ...
  make_option(c("-c", "--clustermapping"), metavar = "file", 
              default = "inputs/processed/allTP2.Rds", help = "TP2 mapping"))
...
if (num_cl == 0) {
  ...
  file.path("intermediate_data", params$int_type[2], "cluster_labs.Rds") %>% saveRDS(clusters, .)
  
  outputDetails(paste0("Collecting pairwise distances (from non-redundant set), ", 
                       "for each of the \n     ", nrow(clusters)-1, " clusters ", 
                       "from user input (top 5 by size and top 5 by rate (potential overlap)) ", 
                       "\n          ", "Top ", num_cl, " by size: ", 
