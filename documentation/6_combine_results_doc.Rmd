...
res_dir <- file.path("results", params$int_type[2])
assert("Interval type directory in results folder exists", dir.exists(res_dir))
dir.create(file.path(res_dir, "Merged_strain_results"), showWarnings = FALSE)
...
results_files <- list.files(res_dir, full.names = TRUE)
...
step7 <- step6 %>% add_column(novel_only_ECCs = 0) %>% 
  select(interval, actual_tp1_cl, tp1_cl, TP1_Size, all_of(tp1eccs), 
         ...
         grep("Novels_only", colnames(step6), value = TRUE), 
...
size_colnames <- colnames(step7) %>% grep("size|Size", ., value = TRUE)
size_cols <- step7[,..size_colnames]
z <- lapply(size_colnames, function(x) {
  y <- size_cols[,..x] %>% pull() %>% na.omit()
  assert(paste0("Column ", x, " is all integers or NA"), all(y == floor(y)))
})
...
writeData(fp = file.path("results", params$int_type[2], "Wide_merged_cluster_results.tsv"), df = step8)
...
step9 %>% arrange(interval, `Actual TP1 cluster`, variable) %>% 
  writeData(fp = file.path("results", params$int_type[2], "Long_merged_cluster_results.tsv"), df = .)
...
cat(paste0("Saving strain files to ", file.path(res_dir, "Merged_strain_results"), " ... \n"))
...
for (i in 2:length(ivls)) {
  ...
  if ("pango_clusters" %in% names(timeN)) {
    step10 <- left_join(step10, timeN$strain_pango, by = "Strain")
  }
  
  writeData(fp = file.path(res_dir, "Merged_strain_results", paste0(ivl_i, ".tsv")), df = step10)
  rm(step10)
}

cat(paste0("See '", res_dir, "' folder for cluster-specific and strain-specific files.\n"))
