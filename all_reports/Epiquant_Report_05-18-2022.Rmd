...
params:
  ...
  heatmap_dir: "results/Multiset/heatmaps/"
  freq_dir: "results/Multiset/frequencies/"
  dens_dir: "results/Multiset/densities/"
  clust_dir: "results/Multiset/heatmaps/clustering/"
---
...
# lines that should be commented out / removed before git updates are marked with : 
...
libs <- c("openxlsx", "reshape2", "plyr", "dplyr", "tidyr", "grid", "gridExtra", 
          ...
          "data.table", "forcats", "plotly", "magick", "filesstrings")
...
## Create figure caption variables (and source in functions)
source("captions_and_functions.R")

# LOADING DATA
Lineages <- read_csv(params$lineage_file, col_types = cols()) %>% 
  filter(T0 != "#N/A") %>% select("Strain", "T0.original", "Pango_lineage")

ECC_long_base <- file.path("results", "Multiset", params$strain_results) %>%
  list.files(., full.names = TRUE, pattern = "tsv") %>%
  map(read_tsv, col_types = cols()) %>% reduce(rbind)

ECC_Month_base <- file.path("results", "YearMonth", params$strain_results) %>% 
  list.files(., full.names = TRUE, pattern = "tsv") %>% 
  map(read_tsv, col_types = cols()) %>% reduce(rbind)
...
ECC_Month_base <- merge(x = ECC_Month_base, y = Lineages, all.x = TRUE, 
                        by = intersect(colnames(ECC_Month_base), colnames(Lineages)))


ECC_Week <- file.path("results", "YearWeek", params$strain_results) %>% 
  list.files(., full.names = TRUE, pattern = "tsv") %>% 
  map(read_tsv, col_types = cols()) %>% reduce(rbind) %>% 
...
ECC_Week <- ECC_Week %>% 
  merge(x = ., y = Lineages, by = intersect(colnames(ECC_Week), colnames(Lineages)), all.x = TRUE) %>% 
...
country_code <- read_csv(params$country_code_file, col_types = cols())
Vector_Order <- read_csv(params$vector_file, col_types = cols())
```


```{r }
# create new variables Timepoints, Multistrains, Week and Running day
ECC_long <- merge(x = ECC_long_base, y = Lineages, all.x = TRUE, 
              by = intersect(colnames(ECC_long_base), colnames(Lineages))) %>% 
...
## Create figure caption variables - source("captions_and_functions.R")
```
...
Whisker_Data <- ECC_long %>% 
  select (Timepoint, "Actual_growth by rate", "Novel_growth by rate") %>% drop_na() %>% 
...
# library(magick); library(filesstrings)
## top5 cluster by TP2 size and adding Pango lineages names, 
## change the file names of the heatmaps to have pango lineages
tpn <- readRDS("inputs/processed/allTP2.Rds")

Lineages_1 <- tpn$original[, c("Strain", "T0")] %>% as.data.table() %>%
...
Lineages_1 <- merge.data.table(Lineages_1, sizes, by = c("Original_colname")) %>% 
  ...
  left_join(Lineages, by = "Strain") %>% select(-Strain) %>% distinct()


TP2_lineage <- ECC_long %>% #filter(Timepoint=="TP2") %>% 
  select(T0.original, `Last time this cluster was seen in TP2`, 
         `Novel_growth by rate`, `Actual_growth by rate`, `Actual TP2 cluster`) %>% 
  distinct() %>% 
  left_join(Lineages_1, by = c("Actual TP2 cluster" = "Original_colname", "T0.original")) %>% 
  rename(Original_colname = `Actual TP2 cluster`)

TP2_lineage <- TP2_lineage %>% 
  mutate(TP2_Cluster = paste0(TP2_lineage$`Last time this cluster was seen in TP2`)) %>% #,
                                   # "-", TP2_lineage$Original_colname)) %>%
  select(TP2_Cluster, Original_colname, Pango_lineage, Size, 
         `Actual_growth by rate`, `Novel_growth by rate`, T0.original)


TP2_names <- list.files(params$clust_dir, full.names = T) %>% 
  lapply(., function(x_i) {
    cluster_label <- strsplit(x_i, split = "/")[[1]][5]
    substr(cluster_label, 15, nchar(cluster_label)-4)
}) %>% unlist() %>% data.frame() %>% setNames("T0.original")

heatmap_names <- TP2_lineage %>% filter(T0.original %in% TP2_names$T0.original)

T5_Size_names <- heatmap_names %>% select(Original_colname, T0.original, Size) %>% distinct() %>% 
  top_n(5) %>% arrange(desc(Size))

# CHECK SIZES
part1 <- TP2_lineage %>% filter(Original_colname %in% T5_Size_names$Original_colname) %>% 
  select(T0.original, Original_colname, Size) %>% arrange(T0.original) %>% distinct()
part2 <- T5_Size_names %>% select(T0.original, Original_colname, Size) %>% arrange(T0.original)
testit::assert("Sizes match", identical(part1, part2))
...
Cluster_heatmap <- function(Cluster_Name, Pango_lineage, Size, heatmap_dir){
  ...
  HF_images <- image_read(Heatmap_files) %>% image_scale("1000x1000")
    image_append(c(HF_images[1], HF_images[3], HF_images[2])) %>% 
     image_annotate(paste0(Pango_lineage, "( n=", Size, " ):", " Geo distance(left), ", 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Size_names$T0.original[5], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Size_names$T0.original[4], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Size_names$T0.original[3], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Size_names$T0.original[2], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Size_names$T0.original[1], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Size_names$T0.original[5], 
                  Pango_lineage = T5_Size_names$T0.original[5], 
                  Size = T5_Size_names$Size[5], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.1.G. **`r paste0(Fig.2.1.G)`**  {#Fig.2.1.G-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Size_names$T0.original[4], 
                  Pango_lineage = T5_Size_names$T0.original[4], 
                  Size = T5_Size_names$Size[4], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.1.H. **`r paste0(Fig.2.1.H)`**  {#Fig.2.1.H-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Size_names$T0.original[3], 
                  Pango_lineage = T5_Size_names$T0.original[3], 
                  Size = T5_Size_names$Size[3], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.1.I. **`r paste0(Fig.2.1.I)`**  {#Fig.2.1.I-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Size_names$T0.original[2], 
                  Pango_lineage = T5_Size_names$T0.original[2], 
                  Size = T5_Size_names$Size[2], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.1.J. **`r paste0(Fig.2.1.J)`**  {#Fig.2.1.J-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Size_names$T0.original[1], 
                  Pango_lineage = T5_Size_names$T0.original[1], 
                  Size = T5_Size_names$Size[1], freq_dir = params$freq_dir)
```
...
```{r}
## top5 growth rate
T5_Growth_names <- heatmap_names %>% 
  select(TP2_Cluster, T0.original, Size, `Novel_growth by rate`) %>% distinct() %>% 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Growth_names$T0.original[5], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Growth_names$T0.original[4], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Growth_names$T0.original[3], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Growth_names$T0.original[2], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_heatmap(Cluster_Name = T5_Growth_names$T0.original[1], 
...
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Growth_names$T0.original[5], 
                  Pango_lineage = T5_Growth_names$T0.original[5], 
                  Size = T5_Growth_names$Size[5], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.2.G. **`r paste0(Fig.2.2.G)`**  {#Fig.2.2.G-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Growth_names$T0.original[4], 
                  Pango_lineage = T5_Growth_names$T0.original[4], 
                  Size = T5_Growth_names$Size[4], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.2.H. **`r paste0(Fig.2.2.H)`**  {#Fig.2.2.H-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Growth_names$T0.original[3], 
                  Pango_lineage = T5_Growth_names$T0.original[3], 
                  Size = T5_Growth_names$Size[3], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.2.I. **`r paste0(Fig.2.2.I)`**  {#Fig.2.2.I-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Growth_names$T0.original[2], 
                  Pango_lineage = T5_Growth_names$T0.original[2], 
                  Size = T5_Growth_names$Size[2], freq_dir = params$freq_dir)
```


##### &nbsp;&nbsp; Figure 2.2.J. **`r paste0(Fig.2.2.J)`**  {#Fig.2.2.J-link -}
```{r fig.height= 9, fig.width= 18 }
Cluster_frequency(Cluster_Name = T5_Growth_names$T0.original[1], 
                  Pango_lineage = T5_Growth_names$T0.original[1], 
                  Size = T5_Growth_names$Size[1], freq_dir = params$freq_dir)
```
...
T5_Size_names <- TP2_lineage %>% select(TP2_Cluster, Pango_lineage, Size) %>% 
  distinct() %>% arrange(desc(Size)) %>% 
  # REMOVE THIS LINE BEFORE SUBMISSION
  filter(Size <= 2000) %>% 
  top_n(5)

#Pattern <- paste0(T5_Size_names$TP2_Cluster_Size [1:5], "|", T5_TP2_names$TP2_Cluster_Size[2]  )
Pattern <- paste0(T5_Size_names$TP2_Cluster[1], "|", T5_Size_names$TP2_Cluster[2], "|", 
                  T5_Size_names$TP2_Cluster[3], "|", T5_Size_names$TP2_Cluster[4], "|", 
                  T5_Size_names$TP2_Cluster[5])
...
T5_Size_names <- TP2_lineage %>% 
  select(TP2_Cluster, Pango_lineage, `Novel_growth by rate`) %>% 
  arrange(desc(`Novel_growth by rate`)) %>% top_n(5)
...
kable(Table2a, escape = FALSE, format = 'html') %>% 
  kableExtra::kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = T) %>% 
